// Kindly borrowed and modified  from assets-webpack-plugin. All credits go to them
// https://github.com/kossnocorp/assets-webpack-plugin/blob/master/lib/output/createOutputWriter.js
...
function error(message, previousError) {
  var err = new Error('[ReactIntlPlugin] ' + message)
...
module.exports = function(options) {
  ...
  return function writeOutput(newAssets, next) {
    ...
    mkdirp(options.path, function(err) {
      if (err) {
        return next(
          error('Could not create output folder ' + options.path, err)
        )
      }
